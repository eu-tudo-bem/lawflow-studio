...
import {
  ...
  MessageSquare, BookOpen, FileSearch, Plus, RefreshCw, Search,
  Eye, Trash2, Gavel
} from "lucide-react";
...
const TJPRMonitor = () => {
  ...
  const handleAdd = async () => {
    ...
    if (!cleaned) {
      toast({ title: "Número inválido", description: "Informe o número do processo.", variant: "destructive" });
      return;
    }

    const { error } = await supabase.from("tjpr_processos_monitorados").insert({
      numero_processo: cleaned,
  ...
  const handleRefresh = async (processoId: string) => {
    ...
    try {
      const { error } = await supabase.functions.invoke("tjpr-monitor", {
        body: { processo_id: processoId },
      });
      if (error) throw error;
      toast({ title: "Consulta realizada", description: "Status atualizado." });
      loadProcessos();
    } catch (err: any) {
  ...
  const handleRefreshAll = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast({ title: "Consulta em lote realizada" });
      loadProcessos();
    } catch (err: any) {
  ...
  const viewLogs = async (processoId: string, numero: string) => {
    ...
    const { data } = await supabase
      .from("tjpr_logs_consulta")
      .select("id, status_anterior, status_novo, erro, sucesso, created_at")
      .eq("processo_id", processoId)
      .order("created_at", { ascending: false })
      .limit(20);
    setSelectedLogs((data || []) as LogConsulta[]);
    ...
    <div className="min-h-screen bg-muted">
        ...
        <nav className="p-4 space-y-1">
          {menuItems.map((item) => (
            <Button
              key={item.label}
              variant="ghost"
              className={`w-full justify-start gap-3 text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground ${item.href === "/dashboard/tjpr" ? "bg-sidebar-accent text-sidebar-accent-foreground" : ""}`}
      ...
      <main className="lg:pl-64">
          ...
          <div className="flex items-center justify-between px-4 lg:px-8 h-16">
            ...
            <div className="flex gap-2">
              ...
              <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Cadastrar Processo para Monitoramento</DialogTitle>
                  </DialogHeader>
                  <div className="space-y-4">
                    <div>
                      <Label>Número do Processo (somente números)</Label>
                      <Input
                        placeholder="00123456720258160001"
        ...
        <div className="p-4 lg:p-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <Card className="border-0 shadow-card">
              <CardContent className="p-4 text-center">
                <p className="text-2xl font-bold text-green-600">{processos.filter(p => p.status_atual?.toLowerCase().includes("ativo") || p.status_atual?.toLowerCase().includes("andamento")).length}</p>
                <p className="text-sm text-muted-foreground">Ativos</p>
              </CardContent>
            </Card>
            <Card className="border-0 shadow-card">
              <CardContent className="p-4 text-center">
                <p className="text-2xl font-bold text-yellow-600">{processos.filter(p => p.status_atual?.toLowerCase().includes("suspenso")).length}</p>
                <p className="text-sm text-muted-foreground">Suspensos</p>
              </CardContent>
            </Card>
            <Card className="border-0 shadow-card">
              <CardContent className="p-4 text-center">
                <p className="text-2xl font-bold text-muted-foreground">{processos.filter(p => p.status_atual?.toLowerCase().includes("arquivado") || p.status_atual?.toLowerCase().includes("baixado")).length}</p>
          ...
          <Card className="border-0 shadow-card">
            <CardHeader>
              <CardTitle className="font-serif">Processos Monitorados</CardTitle>
            </CardHeader>
            <CardContent>
              {loadingData ? (
                    ...
                    <TableBody>
                      {processos.map((p) => (
                        <TableRow key={p.id}>
                          ...
                          <TableCell>
                            <Badge className={getStatusColor(p.status_atual)} variant="outline">
                              {p.status_atual || "Desconhecido"}
                            </Badge>
                          </TableCell>
                          ...
                          <TableCell>
                            {p.ultima_verificacao
                              ? new Date(p.ultima_verificacao).toLocaleDateString("pt-BR", {
                          ...
                          <TableCell className="text-right">
                            <div className="flex gap-1 justify-end">
                              ...
                              <Button
                                variant="ghost"
                                size="icon"
                                onClick={() => viewLogs(p.id, p.numero_processo)}
                                title="Ver histórico"
                              >
      ...
      <Dialog open={logsDialogOpen} onOpenChange={setLogsDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Histórico de Consultas — {selectedProcessoNum}</DialogTitle>
          </DialogHeader>
                ...
                <TableBody>
                  {selectedLogs.map((log) => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell>
                        {log.sucesso ? (
                          <Badge className="bg-green-500/15 text-green-700 border-green-300" variant="outline">Sucesso</Badge>
                        ) : (
                          <Badge variant="destructive" title={log.erro || undefined}>Erro</Badge>
                        )}
                      </TableCell>
                    </TableRow>
                  ))}
