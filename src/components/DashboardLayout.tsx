...
import {
  ...
  Menu, X, MessageSquare, BookOpen, FileSearch, Gavel, LayoutDashboard, Radar, FileText, Activity
} from "lucide-react";
...
const menuItems = [
  ...
  { icon: Activity, label: "Performance (RUM)", href: "/dashboard/web-vitals" },
];
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Button key={item.label} variant="ghost"
                className={`w-full justify-start gap-3 ${isActive ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"}`}
